...
在BrokerController启动时会检查broker是否打开了DLedger，如果启动，则会初始化DLedgerRoleChangeHandler对象，其作用就是当节点的角色发生变化时将DLedger中的角色透传给broker，DLedgerRoleChangeHandler重写了DLedgerLeaderElector.RoleChangeHandler的handle方法，该方法的作用时在节点角色发生变化时broker端相应的处理。



### 2.9.1 主备切换DLeger机制

在 RocketMQ 4.5 版本之前，RocketMQ 只有 Master/Slave 一种部署方式，一组 broker 中有一个 Master ，有零到多个 Slave，Slave 通过同步复制或异步复制的方式去同步 Master 数据。Master/Slave 部署模式，提供了一定的高可用性。但这样的部署模式，有一定缺陷。比如故障转移方面，如果主节点挂了，还需要人为手动进行重启或者切换，无法自动将一个从节点转换为主节点。而 DLedger 就是一个基于 raft 协议的 commitlog 存储库，也是 RocketMQ 实现新的高可用多副本架构的关键。
...
从其基本结构可以看到，DLedgerCommitLog继承自CommitLog，基本实现和之前类似，这里增加了主备切换相关功能。

再看下其文件存储结构

![029](./img/029.png)

这里可以看到DLedgerCommitLog是将原有的commilog当作一个body。先看下基本的存储流程

在DefaultMessageStore#putMessage方法中
...
这里如果开启了主备切换机制，那么这里的commitlog就是DLedgerCommitLog
...
这里基本的写入就不详述了。具体的存入是由DLedgerServer来完成，基于DLedger 实现，它现在是在 OpenMessaging 下的一个项目。大致存储流程如下

![030](./img/030.png)

这里相关的主从复制后面详述，先看下选主流程

### 2.9.2 选主流程

通过上面可知DledgerCommitLog具备了选主功能
...
这里BrokerController启动时会启动DledgerCommitLog，于是启动了存储模块、通信模块、日志复制模块、选举模块。
...
这里StateMaintainer的run方法就是循环调用doWork方法，其中主要做了两件事，一个是刷新心跳相关字段（`heartBeatTimeIntervalMs、maxHeartBeatLeak、minVoteIntervalMs、maxVoteIntervalMs`）；另一个是执行maintainState，这是一个核心方法。这里每次选主后休息10ms。
...
private void maintainState() throws Exception {
  if (memberState.isLeader()) {
    ...
    // 从节点，该状态下会开启定时器，尝试进入到candidate状态，以便发起投票，同时一旦收到
    // 主节点的心跳包，则重置定时器
    maintainAsFollower();
  } else {
    // 候选者，此状态下会发起投票，尝试选择自己为主节点，选举成功后不会存在该状态下的节点
    maintainAsCandidate();
  }
}
```
...
private void maintainAsCandidate() throws Exception {
  ...
  synchronized (memberState) {
    ...
    // lastParseResult的值为ParseResult.WAIT_TO_REVOTE，这个是在节点启动过程在初始化
    // DledgerServer的同时初始化DledgerLeaderElector，就是这个时候对lastParseResult赋值的
...
maintainAsCandidate()会通过调用voteForQuorumResponses方法向集群的节点发送投票请求，然后对投票请求的结果进行统计，如果投票成功则会调用changeRoleToLeader方法将当前节点的角色修改leader，此时由于节点的角色发生变化，所以会通过DLedgerRoleChangeHandler将角色透传给broker，broker此时会开始处理slave数据同步、调度服务、事务消息服务，最终将节点的角色修改为`BrokerRole.SYNC_MASTER`。

voteForQuorumResponses方法会遍历MemberState中的peerMap（peerMap的结构是<peerSelfId, peerAddress>）并构建VoteRequest，**VoteRequest中会包含其group、leader节点的任期值、下一条日志开始的index、当前节点的任期值、当前节点的selfId、集群中其余节点的selfId**。在发送VoteRequest前会先判断请求中远端节点的selfId是否与当前节点相同，也就是该请求是否是发送给自己的，如果是则调用其handleVote方法处理不需要通过网络传输请求否则通过网络传输请求
...
public CompletableFuture<VoteResponse> handleVote(VoteRequest request, boolean self) {
  ...
  synchronized (memberState) {
    //检查请求中的leaderId是否在集群中，返回未知结果，异常情况
    if (!memberState.isPeerMember(request.getLeaderId())) {
      logger.warn("[BUG] [HandleVote] remoteId={} is an unknown member", request.getLeaderId());
      return CompletableFuture.completedFuture(new VoteResponse(request).term(memberState.currTerm()).voteResult(VoteResponse.RESULT.REJECT_UNKNOWN_LEADER));
    }
    //如果该请求不是发给自己的但是请求中的leaderId与自己的selfId相同，异常情况
    if (!self && memberState.getSelfId().equals(request.getLeaderId())) {
      logger.warn("[BUG] [HandleVote] selfId={} but remoteId={}", memberState.getSelfId(), request.getLeaderId());
      return CompletableFuture.completedFuture(new VoteResponse(request).term(memberState.currTerm()).voteResult(VoteResponse.RESULT.REJECT_UNEXPECTED_LEADER));
    }
    //请求的ledgerEndTerm小于自己的ledgerEndTerm，这种情况拒绝投票，也就是说在raft协议中谁的term越大
    // 谁话语权越大
    if (request.getLedgerEndTerm() < memberState.getLedgerEndTerm()) {
      return CompletableFuture.completedFuture(new VoteResponse(request).term(memberState.currTerm()).voteResult(VoteResponse.RESULT.REJECT_EXPIRED_LEDGER_TERM));
    } else if (request.getLedgerEndTerm() == memberState.getLedgerEndTerm() && request.getLedgerEndIndex() < memberState.getLedgerEndIndex()) {
      //请求的ledgerEndTerm与自己的ledgerEndTerm相等但是请求的ledgerEndIndex小于自己的
      //也就是处于同一投票轮次中，地位平等，但是ledgerEndIndex较小，则也是拒绝投票
    ...
    //请求中的term小于当前节点的ledgerEndTerm，也是拒绝投票给请求节点
    if (request.getTerm() < memberState.getLedgerEndTerm()) {
      return CompletableFuture.completedFuture(new VoteResponse(request).term(memberState.getLedgerEndTerm()).voteResult(VoteResponse.RESULT.REJECT_TERM_SMALL_THAN_LEDGER));
    }
    // 请求的LedgerEndIndex小于当前节点也是拒绝
...
* needIncreaseTermImmediately 是否应该立即发起投票。如果为 true，则忽略计时器，该值默认为 false，当收到从主节点的心跳包并且当前状态机的轮次大于主节点的轮次，说明集群中 Leade r的投票轮次小于从当前收到请求节点的投票轮次，应该立即发起新的投票。
* term 投票轮次。
...
- ACCEPT 赞成票，acceptedNum 加一，只有得到的赞成票超过集群节点数量的一半才能成为 Leader。
- REJECT_ALREADY_VOTED 拒绝票，原因是已经投了其他节点的票。
- REJECT_ALREADY_HAS_LEADER 拒绝票，原因是因为集群中已经存在 Leaer了。alreadyHasLeader 设置为true，无需在判断其他投票结果了，结束本轮投票。
- REJECT_TERM_SMALL_THAN_LEDGER 拒绝票，如果自己维护的 term 小于远端维护的 ledgerEndTerm，则返回该结果，如果对端的 team大于自己的 team，需要记录对端最大的投票轮次，以便更新自己的投票轮次。
- REJECT_EXPIRED_VOTE_TERM 拒绝票，如果自己维护的 term 小于远端维护的 term，更新自己维护的投票轮次。
- REJECT_EXPIRED_LEDGER_TERM 拒绝票，如果自己维护的 ledgerTerm 小于对端维护的 ledgerTerm，则返回该结果。如果是此种情况，增加计数器 biggerLedgerNum 的值。
- REJECT_SMALL_LEDGER_END_INDEX 拒绝票，如果对端的 ledgerTeam 与自己维护的 ledgerTeam 相等，但是自己维护的 dedgerEndIndex 小于对端维护的值，返回该值，增加 biggerLedgerNum 计数器的值。
- REJECT_TERM_NOT_READY 拒绝票，对端的投票轮次小于自己的 team，则认为对端还未准备好投票，对端使用自己的投票轮次，是自己进入到 Candidate 状态。

主要投票逻辑点就在投票轮次和复制进度，投票轮次越高优先级也高，复制进度越大优先级越高。最后统计大多数节点赞成的节点称为主节点，一般集群中broker数量最好为奇数。

最终选出主节点后，主节点会调用maintainAsLeader方法，主要任务就是发送心跳信息给集群中的其他节点。而对于从节点，则会执行maintainAsFollow方法，主要逻辑就是判断上次发送收到心跳信息的时间到现在为止是否超时且没有收到心跳信息，如果是则会将其状态修改为candidate。
...
因为定时器会定时检查状态，如果是candidate，则会执行相关选主方法，于是在主节点宕机时通过心跳时间判断是否需要重新选主。
...
在消息存储处理类`DefaultMessageStore#start`方法中会启动`HAService`主从同步服务。

**消息异步传输**

如果master broker 的角色是 `ASYNC_MASTER`，那么消息等待从 Master 同步到Slave到方式是**异步传输**方式。表示消息发送到Master时，master在存储完这条消息到本地之后，并不会等待消息同步到Slave才返回码，此时会立刻反馈生产者。

**消息同步传输**

如果采用同步传输 `SYNC_MASTER`，此时反馈生产者时需要满足**两个条件**

> 1、消息体属性 PROPERTY_WAIT_STORE_MSG_OK 为true，表示允许等待
>
> 2、Slave相比Master落后的进度不能超过256M

在从Master同步到Slave之后再反馈给生产者。当然如果不满足上面两个条件，那master就不会再等待消息同步到slave之后再返回，而是能尽早返回便尽早返回。

消息存储CommitLog#putMessage最后
...
这里有两个操作，一个是检查是否要刷盘（异步/同步），一个是主从复制。下面即使主从复制的详细逻辑
...
`AcceptSocketService` 服务的功能是 Master 等待接受来自其它客户端 Slave 的连接，当成功建立连接后，会将这条连接 `HAConnection` 放入到 `connectionList` 连接列表里面。而 `HAClient` 服务的功能是 Slave 主动发起同Master 的连接。当`HAService`启动之后，一旦Master发现和Slave不同步，那么Master会自动开始同步消息到Slave，无序再次触发。
...
这里很明显可以看到从节点开启了一个同步定时任务，每10s向主节点发送一次同步请求。
...
但是可以看到，这里只是同步了元数据信息，没有同步具体的消息数据。前面说到broker启动时在DefaultMessageStore中会启动一个HAService服务，这是一个小型服务，运行在后台。
...
HASerivce服务端会保存每一个请求连接的客户端HALClient。而客户端会首先和master建立长连接（中途断开会重连），然后循环向master反馈相关同步信息，然后从master拉取消息数据，从这里可知，这里才是同步的主要流程，而消息写入时只是做一个兜底检查。这里就不再对具体逻辑代码做说明了。

疑问点：master和slave都是broker服务，但是代码中在启动HAService和HAClient的时候却并没有看到区分两者的判断。比如连接master的时候，如果本身就是master，那就没必要连接自己了？

同时我们可以看到这里的逻辑是不支持主备切换方式下的主从复制，其实支持主备切换方式下的主从复制上文已经有大概说明了，后面再详述。而这里如何保证主从数据一致性呢？
...
1、maste、slave都在运行过程中，消息消费者是从master拉取消息还是从slave拉取？

答：默认情况下，RocketMQ消息消费者从master拉取，当master积压的消息超过了物理内存的40%，则建议从slave拉取。但如果slaveReadEnable为false，表示slave不可读，此时slave也不会接管消息拉取。

2、当消息消费者向slave拉取消息后，会一直从slave拉取？

答：不是的。分如下情况：
1）如果slave的slaveReadEnable设置为false，则下次从master拉取。

2）如果slave允许读取并且积压的消息未超过其物理内存的40%，下次拉取使用的Broker为订阅组的brokerId指定的Broker服务器，该值默认为0，代表master。

3）如果slave允许读取并且积压的消息超过了其物理内存的40%，下次拉取使用的Broker为订阅组的whichBrokerWhenConsumeSlowly指定的Broker服务器，该值默认为1，代表从服务器。
...
这里其实之前已经说明了，这里再次回顾下，这种模式下面broker启动时不会创建HAService服务，而是在每次消息写入的时候都需要检查，超过半数的slave同步成功后才会返回成功。

疑问点：两种模式如何选择，如果选择支持主备切换模式，主从同步效率如何保证？两种模式如何选择？



相关Dledger模式后面再详细研究，推荐使用更高版本的RocketMQ。参考
...
> DLedger模型在4.8.0之前的版本，没有主从同步双写或异步消息复制的区别，都是同步阻塞方式，在DLedgerCommitLog类里，都是调用的putMessage方法，没有异步化的操作。所以建议升级4.8.0版本。

> 老版本不支持堆外内存
> DLedger模型的另一个问题是在put message的时候不支持使用堆外内存，这个问题目前在4.8.0中还没修复.非DLedger通过配置参数：transientStorePoolEnable可以选择启用堆外内存，默认会额外占用5个G，发送消息处理当提交消息的时候可以先写入堆外内存，异步写入page cache,然后刷盘等，读取还是从page cache，实现读写分离。DLedger目前还是通过mmap直接写入page cache。
