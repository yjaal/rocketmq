...
```shell
# 1.启动NameServer nohup ... & 表明让程序在后台执行
...
```shell
# 1.启动Broker,这里需要将broker默认配置修改下，将最后的aclEnable=true注释掉
...
- `Broker`部署相对复杂，`Broker`分为`Master`与`Slave`，一个Master可以对应多个Slave，但是一个Slave只能对应一个Master，Master与Slave的对应关系通过指定相同的BrokerName，不同的BrokerId来定义，BrokerId为0表示Master，非0表示Slave。Master也可以部署多个。每个Broker与NameServer集群中的所有节点建立长连接，定时注册Topic信息到所有NameServer。
- Producer与NameServer集群中的其中一个节点（随机选择）建立长连接，定期从NameServer取Topic路由信息，并向提供Topic服务的Master建立长连接，且定时向Master发送心跳。Producer完全无状态，可集群部署。
- Consumer与NameServer集群中的其中一个节点（随机选择）建立长连接，定期从NameServer取Topic路由信息，并向提供Topic服务的Master、Slave建立长连接，且定时向Master、Slave发送心跳。Consumer既可以从Master订阅消息，也可以从Slave订阅消息，订阅规则由Broker配置决定。注意和Producer的区别。
...
| **序号** | **IP**       | **角色**                 | **架构模式**    |
| -------- | ------------ | ------------------------ | --------------- |
| 1        | 192.168.67.2 | nameserver、brokerserver | Master1、Slave2 |
| 2        | 192.168.67.3 | nameserver、brokerserver | Master2、Slave1 |
...
```bash
# nameserver
192.168.67.2 rocketmq-nameserver1
192.168.67.3 rocketmq-nameserver2
# broker
192.168.67.2 rocketmq-master1
192.168.67.2 rocketmq-slave2
192.168.67.3 rocketmq-master2
192.168.67.3 rocketmq-slave1
```
...
或者为了安全，只开放特定的端口号，RocketMQ默认使用3个端口：9876 、10911 、11011 、10909、10912。如果防火墙没有关闭的话，那么防火墙就必须开放这些端口：
...
注意：对于`ubunut20 `可以安装ufw进行设置，参考:`https://www.zhangshengrong.com/p/QrXeb6B41d/`
...
## 我本地
ROCKETMQ_HOME=/home/yj/rocketmq/rocketmq-all-4.5.1
PATH=$PATH:$ROCKETMQ_HOME/bin
export ROCKETMQ_HOME PATH
```
...
## 我本地
mkdir /home/yj/rocketmq/store_master
mkdir /home/yj/rocketmq/store_master/commitlog
mkdir /home/yj/rocketmq/store_master/consumequeue
mkdir /home/yj/rocketmq/store_master/index

mkdir /home/yj/rocketmq/store_slave
mkdir /home/yj/rocketmq/store_slave/commitlog
mkdir /home/yj/rocketmq/store_slave/consumequeue
mkdir /home/yj/rocketmq/store_slave/index
```

### 3.3.8 broker配置文件

**注意：这里由于master1和slave2在同一台机器上面，所以相关配置的地址不能相同，要区分，不然broker无法启动**
...
## 我本地
vi /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-a.properties
```
...
#Broker 对外服务的监听端口，还有一个vip端口，就是下面端口减2
listenPort=10911
#删除文件时间点，默认凌晨 4点
deleteWhen=04
#文件保留时间，默认 48 小时
...
#存储路径
storePathRootDir=/home/yj/rocketmq/store_master
#commitLog 存储路径
storePathCommitLog=/home/yj/rocketmq/store_master/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/home/yj/rocketmq/store_master/consumequeue
#消息索引存储路径
storePathIndex=/home/yj/rocketmq/store_master/index
#checkpoint 文件存储路径
storeCheckpoint=/home/yj/rocketmq/store_master/checkpoint
#abort 文件存储路径
abortFile=/home/yj/rocketmq/store_master/abort
#限制的消息大小
...
```sh
vi /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-b-s.properties
```
...
diskMaxUsedSpaceRatio=88
#存储路径
storePathRootDir=/home/yj/rocketmq/store_slave
#commitLog 存储路径
storePathCommitLog=/home/yj/rocketmq/store_slave/commitlog
#消费队列存储路径存储路径
storePathConsumeQueue=/home/yj/rocketmq/store_slave/consumequeue
#消息索引存储路径
storePathIndex=/home/yj/rocketmq/store_slave/index
#checkpoint 文件存储路径
storeCheckpoint=/home/yj/rocketmq/store_slave/checkpoint
#abort 文件存储路径
abortFile=/home/yj/rocketmq/store_slave/abort
#限制的消息大小
maxMessageSize=65536
#flushCommitLogLeastPages=4
#flushConsumeQueueLeastPages=2
...
```sh
vi /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-b.properties
```
...
```sh
vi /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-a-s.properties
```
...
```sh
vi /home/yj/rocketmq/rocketmq-all-4.5.1/bin/runbroker.sh
```
...
```sh
vim /home/yj/rocketmq/rocketmq-all-4.5.1/bin/runserver.sh
```
...
分别在192.168.67.2和192.168.67.3启动NameServer

```bash
cd /home/yj/rocketmq/rocketmq-all-4.5.1/bin
nohup sh mqnamesrv &
```

#### 2）启动Broker集群

* 在192.168.67.2上启动master1和slave2

master1：

```bash
cd /home/yj/rocketmq/rocketmq-all-4.5.1/bin
nohup sh mqbroker -c /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-a.properties &
```
...
```sh
cd /home/yj/rocketmq/rocketmq-all-4.5.1/bin
nohup sh mqbroker -c /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-b-s.properties &
```

* 在192.168.67.3上启动master2和slave2

master2

```sh
cd /home/yj/rocketmq/rocketmq-all-4.5.1/bin
nohup sh mqbroker -c /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-b.properties &
```
...
```sh
cd /home/yj/rocketmq/rocketmq-all-4.5.1/bin
nohup sh mqbroker -c /home/yj/rocketmq/rocketmq-all-4.5.1/conf/2m-2s-sync/broker-a-s.properties &
```
...
```sh
rocketmq.config.namesrvAddr=192.168.67.2:9876;192.168.67.3:9876
```
...
我们可以直接通过源码中的例子进行试验，之前使用的例子就在example包中，首先对原有的例子稍作修改

```java
// example.quickstart包中
...
这种可靠性同步地发送方式使用的比较广泛，比如：重要的消息通知，短信通知。其实上面那个例子就是一个同步发送
...
这种拉取模式DefaultMQPullConsumer在高版本中已经不推荐使用了，可以使用DefaultLitePullConsumer，这样就不用自己去管理队列的偏移量了。

#### 2) Broker推送消息给消费者进行消费
...
这里Broker会将这个组未消费的TopicTest消息推送过来进行消费
...
可以看到每一个队列中queueOffset是有序的，这就是局部有序，而全局无序
...
现在RocketMq并不支持任意时间的延时，需要设置几个固定的延时等级，从1s到2h分别对应着等级1到18。当然这只是开源版本的限制，商业版是没有此限制的。

## 4.4 批量消息

**批量发送消息能显著提高传递小消息的性能。限制是这些批量消息应该有相同的topic，相同waitStoreMsgOK，而且不能是延时消息。此外，这一批消息的总大小不应超过4MB。**
...
在大多数情况下，TAG是一个简单而有用的设计，可以理解为一个子Topic，其可以来选择您想要的消息。例如：
...
这里采用的过滤方案是在broker中进行过滤，过滤完了再往消费者推送
...
* TransactionStatus.Unknown: 中间状态，它代表需要检查消息队列来确定状态，重复查询。
...
这里参考 `./docs/cn/acl/user_guide.md`

在实际使用的时候需要引入相关的权限控制包
...
这里参考 `./docs/cn/msg_trace/user_guide.md`



## 4.7 springboot整合

参考：`https://github.com/apache/rocketmq-spring`
